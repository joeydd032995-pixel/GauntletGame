...
const {mesh:terrain}=createLayeredTerrain({size:240,segments:240,heightFn});scene.add(terrain);
const physics=new PhysicsWorld({terrainHeight:heightFn,terrainNormal:normalFn});
const world=new WorldStreamer({scene,camera,heightFn,chunkSize:30,radius:3});

let seed=712367;const rng=()=>((seed=(seed*48271)%2147483647)/2147483647);
const ruinMat=new THREE.MeshPhysicalMaterial({color:0x66706e,roughness:.76,metalness:.08,clearcoat:.05});
for(let i=0;i<22;i++){const a=i/22*Math.PI*2,r=10.5+(i%3)*.8,x=Math.cos(a)*r,z=Math.sin(a)*r,h=2.2+rng()*3.2,m=new THREE.Mesh(new THREE.BoxGeometry(1.45,h,1.2,2,3,2),ruinMat);m.position.set(x,heightFn(x,z)+h*.5,z);m.rotation.set((rng()-.5)*.08,-a+(rng()-.5)*.35,(rng()-.5)*.07);m.castShadow=m.receiveShadow=true;scene.add(m);physics.addBox(m.position,new THREE.Vector3(.73,h*.5,.61));}
const crystalMat=new THREE.MeshPhysicalMaterial({color:0x7ce7ff,emissive:0x159bd5,emissiveIntensity:4.2,roughness:.09,metalness:.28,clearcoat:.5,transmission:.08,thickness:.3});
for(let i=0;i<16;i++){const a=rng()*6.28,r=13+rng()*31,x=Math.cos(a)*r,z=Math.sin(a)*r,c=new THREE.Mesh(new THREE.OctahedronGeometry(.18+rng()*.35,1),crystalMat);c.scale.y=2.7+rng()*4.4;c.position.set(x,heightFn(x,z)+.62,z);c.rotation.z=(rng()-.5)*.28;c.castShadow=true;scene.add(c);const l=new THREE.PointLight(0x42d7ff,.52,5.5,2);l.position.copy(c.position);scene.add(l);}
...
const enemyRig=createEnemyRig(),enemy=enemyRig.group;enemy.position.set(0,heightFn(0,-6),-6);scene.add(enemy);
...
const atmosphere=createVolumetricAtmosphere({scene,camera,layers:18,radius:120,height:28});
...
const dustCount=900,dustGeo=new THREE.BufferGeometry(),dustPos=new Float32Array(dustCount*3);for(let i=0;i<dustCount;i++){dustPos[i*3]=(rng()-.5)*130;dustPos[i*3+1]=rng()*21;dustPos[i*3+2]=(rng()-.5)*130;}dustGeo.setAttribute('position',new THREE.BufferAttribute(dustPos,3));const dust=new THREE.Points(dustGeo,new THREE.PointsMaterial({color:0xc1e7df,size:.032,transparent:true,opacity:.32,depthWrite:false,blending:THREE.AdditiveBlending}));scene.add(dust);

const keys=new Set();let yaw=0,pitch=.27,dragging=false,lastX=0,lastY=0,stamina=100,hp=100,enemyHp=100,riftT=0,guardT=0,attackT=0,attackWindow=false,dodgeT=0,time=0,lastFacing=0;
...
function update(dt){time+=dt;const simDt=vfx.update(dt);riftT=Math.max(0,riftT-dt);guardT=Math.max(0,guardT-dt);attackT=Math.max(0,attackT-dt);dodgeT=Math.max(0,dodgeT-dt);dust.rotation.y+=dt*.004;atmosphere.update(dt,time);world.update(hero.position);
  ...
  if(attackWindow&&attackT<.35&&attackT>.25){attackWindow=false;if(enemyHp>0&&hero.position.distanceTo(enemy.position)<3.3){enemyHp=Math.max(0,enemyHp-17);enemyAnim.trigger('enemyHit',{fade:.025});vfx.impact(enemy.position.clone().add(new THREE.Vector3(0,1.42,0)),{count:22,scale:1.05});audio.playImpact(enemy.position,1);}}
  ...
  camTarget.copy(hero.position).add(new THREE.Vector3(0,1.62,0));const dist=6.15,cp=Math.cos(pitch),sp=Math.sin(pitch);camDesired.set(Math.sin(yaw)*cp*dist,2.0+sp*dist,Math.cos(yaw)*cp*dist).add(camTarget);if(vfx.impulse>0)camDesired.add(new THREE.Vector3((Math.random()-.5)*vfx.impulse,(Math.random()-.5)*vfx.impulse,(Math.random()-.5)*vfx.impulse));camera.position.lerp(camDesired,1-Math.exp(-dt*13));camera.lookAt(camTarget);
  updateHud(sprint,combat);
}

function updateHud(sprint,combat){document.querySelector('#hp').style.width=`${hp}%`;document.querySelector('#stamina').style.width=`${stamina}%`;document.querySelector('#enemyHp').style.width=`${enemyHp}%`;document.querySelector('#targetCard').classList.toggle('hidden',!combat||enemyHp<=0);document.querySelector('#status').textContent=hp<=0?'DOWNED':guardT>0?'GUARDING':dodgeT>0?'EVADING':sprint?'SPRINTING':combat?'ENGAGED':'READY';document.querySelector('#cd1').textContent=attackT>0?attackT.toFixed(1):'';document.querySelector('#cd2').textContent=riftT>0?riftT.toFixed(1):'';document.querySelector('#cd3').textContent=guardT>0?guardT.toFixed(1):'';const card=document.querySelector('#targetCard');card.dataset.state=enemyBrain.state;}
