...
const capture=new CharacterCaptureDirector({camera,hero,enemy,heroAnim,enemyAnim,heightFn});
...
addEventListener('keydown',e=>{keys.add(e.code);if(e.repeat||capture.active)return;
  ...
  if(e.code==='Space'&&stamina>=22&&dodgeT<=0&&hp>0){stamina-=22;dodgeT=.62;heroAnim.trigger('dodge',{fade:.025});const d=new THREE.Vector3(Math.sin(hero.rotation.y),0,Math.cos(hero.rotation.y));heroBody.velocity.addScaledVector(d,7.5);audio.playWhoosh(hero.position,.72);}
});
addEventListener('keyup',e=>keys.delete(e.code));renderer.domElement.addEventListener('contextmenu',e=>e.preventDefault());renderer.domElement.addEventListener('pointerdown',e=>{if(e.button===2&&!capture.active){dragging=true;lastX=e.clientX;lastY=e.clientY;}});addEventListener('pointerup',()=>dragging=false);addEventListener('pointermove',e=>{if(!dragging)return;cameraRig.orbit(e.clientX-lastX,e.clientY-lastY);yaw=cameraRig.yaw;lastX=e.clientX;lastY=e.clientY;});
...
function update(dt){
  time+=dt;const simDt=vfx.update(dt);riftT=Math.max(0,riftT-dt);guardT=Math.max(0,guardT-dt);attackT=Math.max(0,attackT-dt);dodgeT=Math.max(0,dodgeT-dt);dust.rotation.y+=dt*.004;atmosphere.update(dt,time);world.update(hero.position);
  fwd.set(Math.sin(yaw),0,-Math.cos(yaw));right.set(Math.cos(yaw),0,Math.sin(yaw));move.set(0,0,0);
  if(capture.active){heroBody.desiredVelocity.set(0,0,0);enemyBody.desiredVelocity.set(0,0,0);capture.update(dt);}else{
    if(keys.has('KeyW'))move.add(fwd);if(keys.has('KeyS'))move.sub(fwd);if(keys.has('KeyD'))move.add(right);if(keys.has('KeyA'))move.sub(right);if(move.lengthSq()>0)move.normalize();
    ...
    physics.stepCharacter(heroBody,simDt);hero.position.copy(heroBody.position);heroAnim.setLocomotion({speed:Math.hypot(heroBody.velocity.x,heroBody.velocity.z),angularSpeed,grounded:heroBody.grounded,combat:enemyHp>0&&hero.position.distanceTo(enemy.position)<14,dt:Math.max(simDt,.001)});
  }
  const rootDelta=heroAnim.update(simDt,heroBody.desiredVelocity,fwd);if(!capture.active&&['attack','dodge'].includes(heroAnim.state)&&rootDelta.length()<.9){tmp.set(rootDelta.x,0,rootDelta.z).applyAxisAngle(worldUp,hero.rotation.y);hero.position.add(tmp);heroBody.position.copy(hero.position);}
  if(!capture.active&&attackWindow&&attackT<.42&&attackT>.27){attackWindow=false;if(enemyHp>0&&hero.position.distanceTo(enemy.position)<3.3){enemyHp=Math.max(0,enemyHp-17);enemyAnim.trigger('enemyHit',{fade:.025});vfx.impact(enemy.position.clone().add(new THREE.Vector3(0,1.42,0)),{count:20,scale:1});audio.playImpact(enemy.position,1);}}
  if(!capture.active&&enemyHp<=0&&enemyBrain.state!=='dead'){enemyBrain.kill();enemyTelegraph?.dispose();enemyTelegraph=null;}
  if(!capture.active&&enemyHp>0){tmp.subVectors(hero.position,enemy.position);const targetAngle=Math.atan2(tmp.x,tmp.z),ed=THREE.MathUtils.euclideanModulo(targetAngle-enemy.rotation.y+Math.PI,Math.PI*2)-Math.PI;enemy.rotation.y+=ed*(1-Math.exp(-dt*9));const ai=enemyBrain.update(simDt);if(ai.damage){hp=Math.max(0,hp-(guardT>0?Math.ceil(ai.damage*.3):ai.damage));if(hp>0)heroAnim.trigger('hit',{fade:.025});}
    ...
    physics.stepCharacter(enemyBody,simDt);enemy.position.copy(enemyBody.position);enemyAnim.setLocomotion({speed:Math.hypot(enemyBody.velocity.x,enemyBody.velocity.z),angularSpeed:ed/Math.max(dt,.001),combat:true,grounded:enemyBody.grounded,dt:Math.max(simDt,.001)});
  }
  enemyAnim.update(simDt,enemyBody.desiredVelocity,tmp);
  if(!capture.active&&hp<=0&&heroAnim.state!=='death'){heroAnim.trigger('death',{fade:.08});heroBody.desiredVelocity.set(0,0,0);}
