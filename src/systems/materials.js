...
function makeMaps(key,{base=0x808080,secondary=0x505050,seed=1,size=128,frequency=.08,roughness=.75,roughVariation=.18,normalStrength=2.5,repeat=3}){
  ...
  for(let y=0;y<size;y++)for(let x=0;x<size;x++){const i=y*size+x,n=fbm(x*frequency,y*frequency,seed),grain=rand2(x,y,seed+91),vein=Math.sin((x*.09+y*.035)+n*5)*.5+.5,h=THREE.MathUtils.clamp(n*.76+grain*.12+vein*.12,0,1);heights[i]=h;const mix=THREE.MathUtils.smoothstep(h,.28,.82),c=baseC.clone().lerp(secondC,mix*.55);const lum=.9+grain*.1;const j=i*4;ci.data[j]=Math.round(THREE.MathUtils.clamp(c.r*lum,0,1)*255);ci.data[j+1]=Math.round(THREE.MathUtils.clamp(c.g*lum,0,1)*255);ci.data[j+2]=Math.round(THREE.MathUtils.clamp(c.b*lum,0,1)*255);ci.data[j+3]=255;const rv=THREE.MathUtils.clamp(roughness+(h-.5)*roughVariation+(grain-.5)*.04,0,1);ri.data[j]=ri.data[j+1]=ri.data[j+2]=Math.round(rv*255);ri.data[j+3]=255;}
  ...
  for(const t of[map,normalMap,roughnessMap]){t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(repeat,repeat);t.anisotropy=4;t.generateMipmaps=true;}
...
export const makeStoneMaterial=()=>physicalSurface('stone-osrs-refined',{base:0x918b78,secondary:0x4c5849,seed:29,frequency:.062,roughness:.91,roughVariation:.1,normalStrength:2.45,repeat:1.85,normalScale:.62,metalness:.006,clearcoat:0,envMapIntensity:.22});
export const makeBarkMaterial=()=>physicalSurface('bark-osrs-refined',{base:0x75513b,secondary:0x3e2b22,seed:47,frequency:.11,roughness:.95,roughVariation:.06,normalStrength:2.9,repeat:2.2,normalScale:.76,envMapIntensity:.12});
export const makeFoliageMaterial=()=>physicalSurface('foliage-osrs-refined',{base:0x6d8752,secondary:0x35563a,seed:73,frequency:.085,roughness:.9,roughVariation:.07,normalStrength:1.3,repeat:1.7,normalScale:.32,side:THREE.DoubleSide,emissive:0x000000,emissiveIntensity:0,envMapIntensity:.18});
