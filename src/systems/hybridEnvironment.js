...
export async function installHybridEnvironment({scene,heightFn}){
  ...
  try{
    ...
    const authoredMaterial=new THREE.MeshStandardMaterial({map:texture,color:0xb4b19f,roughness:.94,metalness:.002,envMapIntensity:.14});status.texture={url:TEXTURE_URL,colorSpace:'srgb',authoredAtlas:true};const loader=new OBJLoader(),loaded={};
    ...
    const placements=[
      // Open south approach: no broad wall modules on the combat camera axis.
      ['stairs',{x:0,z:-9.7,rotation:Math.PI,scale:.92,kind:'stairs',yOffset:.01}],
      ...
      ['crates',{x:-4.25,z:-9.55,rotation:.5,scale:.7,kind:'crates'}],['barrel',{x:4.15,z:-9.7,rotation:-.35,scale:.7,kind:'barrel'}],
    ...
    status.landmarks=landmarkAnchors.length;status.suppressedProcedural=suppressProceduralNear(scene,landmarkAnchors,3.4);scene.add(root);status.ready=true;status.root=root.name;return root;
