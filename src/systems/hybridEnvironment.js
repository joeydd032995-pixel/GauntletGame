...
function tune(object,material){
  ...
  object.traverse(node=>{if(!node.isMesh)return;meshes++;const geometry=node.geometry;const count=geometry?.index?.count??geometry?.attributes?.position?.count??0;triangles+=Math.floor(count/3);node.castShadow=true;node.receiveShadow=true;node.material=material;});
...
export async function installHybridEnvironment({scene,heightFn}){
  ...
  try{
    ...
    texture.colorSpace=THREE.SRGBColorSpace;texture.anisotropy=4;texture.wrapS=texture.wrapT=THREE.ClampToEdgeWrapping;texture.needsUpdate=true;
    const authoredMaterial=new THREE.MeshStandardMaterial({map:texture,color:0xffffff,roughness:.86,metalness:.015,envMapIntensity:.32});
    status.texture={url:TEXTURE_URL,colorSpace:'srgb',authoredAtlas:true};
    const loader=new OBJLoader(),loaded={};
