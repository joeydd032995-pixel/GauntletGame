...
export function createShrubGeometry(){
  const parts=[];for(let tier=0;tier<3;tier++){const count=5-tier;for(let i=0;i<count;i++){const a=i/count*Math.PI*2+tier*.58,g=new THREE.PlaneGeometry(.48-tier*.06,.38-tier*.04,1,1);g.translate(0,.16,0);g.rotateX(-.28+tier*.06);g.rotateY(a);g.translate(Math.cos(a)*(.18-tier*.035),.12+tier*.18,Math.sin(a)*(.18-tier*.035));parts.push(g);}}const crown=new THREE.OctahedronGeometry(.27,1);crown.scale(1,.72,1);crown.translate(0,.55,0);if(crown.attributes.uv)crown.deleteAttribute('uv');parts.push(crown);for(const p of parts){if(p.attributes.uv)p.deleteAttribute('uv');if(!p.attributes.normal)p.computeVertexNormals();}const merged=mergeGeometries(parts,false);for(const p of parts)p.dispose();if(!merged)throw new Error('Shrub geometry merge failed');merged.computeVertexNormals();return merged;
}
export function createMossPatchGeometry(){
  const parts=[];for(let i=0;i<5;i++){const a=i/5*Math.PI*2,r=i===0?0:.18,g=new THREE.CircleGeometry(.25-(i%2)*.035,10);g.rotateX(-Math.PI/2);g.translate(Math.cos(a)*r,.006+i*.001,Math.sin(a)*r);parts.push(g);}const merged=mergeGeometries(parts,false);for(const p of parts)p.dispose();if(!merged)throw new Error('Moss patch geometry merge failed');merged.computeVertexNormals();return merged;
}
