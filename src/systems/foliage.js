...
export function createShrubGeometry(){
  const parts=[];for(let tier=0;tier<4;tier++){const count=6-tier;for(let i=0;i<count;i++){const a=i/count*Math.PI*2+tier*.51,g=new THREE.PlaneGeometry(.5-tier*.065,.4-tier*.045,1,1);g.translate(0,.17,0);g.rotateX(-.34+tier*.055);g.rotateZ((i%2?1:-1)*(.08+tier*.02));g.rotateY(a);const r=.22-tier*.035;g.translate(Math.cos(a)*r,.11+tier*.145,Math.sin(a)*r);parts.push(g);}}const merged=mergeGeometries(parts,false);for(const p of parts)p.dispose();if(!merged)throw new Error('Shrub geometry merge failed');merged.computeVertexNormals();return merged;
}
