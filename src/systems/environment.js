...
export function buildRuinArena({scene,physics,heightFn,rng}){
  ...
  const rune=new THREE.Mesh(new THREE.RingGeometry(2.4,2.48,96),new THREE.MeshBasicMaterial({color:0x52cce8,transparent:true,opacity:.26,blending:THREE.AdditiveBlending,depthWrite:false,toneMapped:false}));rune.rotation.x=-Math.PI/2;rune.position.set(0,heightFn(0,0)+.035,0);root.add(rune);
  return root;
}
