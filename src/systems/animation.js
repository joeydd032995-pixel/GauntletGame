...
export class FootIK{
  ...
  update(dt){if(!this.height||!this.feet.length)return;let hipDrop=0;for(const c of this.feet){c.foot.getWorldPosition(this.world);const h=this.height(this.world.x,this.world.z),clearance=this.world.y-h,prev=this.prev.get(c.foot),vertical=Math.abs((this.world.y-prev.y)/Math.max(dt,.001)),lock=this.locks.get(c.foot);const contact=clearance<.11&&vertical<1.25;
      if(contact&&!lock.active){lock.active=true;lock.world.set(this.world.x,h+.045,this.world.z);}if(lock.active&&(clearance>.2||vertical>2.1||this.world.distanceTo(lock.world)>.42))lock.active=false;
      const targetY=h+.045,delta=THREE.MathUtils.clamp(targetY-this.world.y,-.16,.34),r=this.rest.get(c.foot);c.foot.position.y=damp(c.foot.position.y,r.pos.y+delta,22,dt);hipDrop=Math.min(hipDrop,delta);
      if(lock.active){const localTarget=c.foot.parent.worldToLocal(lock.world.clone()),dx=THREE.MathUtils.clamp(localTarget.x-c.foot.position.x,-.14,.14),dz=THREE.MathUtils.clamp(localTarget.z-c.foot.position.z,-.14,.14);c.foot.position.x=damp(c.foot.position.x,r.pos.x+dx,28,dt);c.foot.position.z=damp(c.foot.position.z,r.pos.z+dz,28,dt);}
      ...
      c.foot.getWorldPosition(this.world);if(contact&&lock.active){const slide=Math.hypot(this.world.x-prev.x,this.world.z-prev.z)/Math.max(dt,.001);this.samples++;this.slideSum+=slide;this.slidePeak=Math.max(this.slidePeak,slide);}prev.copy(this.world);
    }if(this.hips){const r=this.rest.get(this.hips).pos;this.hips.position.y=damp(this.hips.position.y,r.y+hipDrop*.52,16,dt);}}
  getTelemetry(){return{contactSamples:this.samples,averageSlide:+(this.samples?this.slideSum/this.samples:0).toFixed(3),peakSlide:+this.slidePeak.toFixed(3),lockedFeet:[...this.locks.values()].filter(x=>x.active).length};}
