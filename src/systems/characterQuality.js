...
export function sanitizeProductionShell(root){
  ...
  root.traverse(o=>{
    ...
    if(vals.some(v=>!Number.isFinite(v))){report.nonFiniteTransforms.push(o.name||o.type);if(!Number.isFinite(o.rotation.x))o.rotation.x=0;if(!Number.isFinite(o.rotation.y))o.rotation.y=0;if(!Number.isFinite(o.rotation.z))o.rotation.z=0;if(!Number.isFinite(o.position.x))o.position.x=0;if(!Number.isFinite(o.position.y))o.position.y=0;if(!Number.isFinite(o.position.z))o.position.z=0;if(!Number.isFinite(o.scale.x))o.scale.x=1;if(!Number.isFinite(o.scale.y))o.scale.y=1;if(!Number.isFinite(o.scale.z))o.scale.z=1;}
    if(!o.isMesh)return;report.meshes++;if(o.userData.productionShell)report.productionMeshes++;
    const g=o.geometry;if(g){if(!g.attributes?.normal)report.missingNormals.push(o.name||'unnamed');const n=g.index?g.index.count/3:(g.attributes?.position?.count||0)/3;report.triangles+=n;g.computeBoundingBox();const s=new THREE.Vector3();g.boundingBox?.getSize(s);if(s.lengthSq()<1e-7)report.tinyMeshes.push(o.name||'unnamed');}
