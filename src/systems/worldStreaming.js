...
export class WorldStreamer{
  constructor({scene,camera,heightFn,chunkSize=28,radius=3}){
    ...
    this.species=createTreeSpeciesLibrary();this.rockMat=makeStoneMaterial();this.rockGeo=new THREE.DodecahedronGeometry(.68,1);this.telemetry={target:'high-end OSRS/07Scape',species:this.species.map(s=>s.id),activeChunks:0,trees:0,rocks:0,nearTreeChunks:0,farTreeChunks:0,lod:'near-card / far-hull',densityProfile:'curated-medium'};this.#publishTelemetry();
  }
  update(focus){
  ...
  #load(cx,cz,key){
    ...
    for(let i=0;i<24;i++){
      ...
      if(kind>.34){
        const roll=hash2(cx*31+i*17,cz*41-i*9,207),speciesIndex=roll<.42?0:roll<.72?1:2,kit=this.species[speciesIndex],slot=chunk.speciesCounts[speciesIndex];if(slot>=chunk.treeCapacity)continue;
        const spread=hash2(i*13,cx+cz,41),s=THREE.MathUtils.lerp(kit.baseScale[0],kit.baseScale[1],spread),yaw=hash2(i,cz,53)*Math.PI*2,lean=(hash2(i,cx,79)-.5)*.045,q=new THREE.Quaternion().setFromEuler(new THREE.Euler(lean,yaw,-lean*.65)),crownYaw=new THREE.Quaternion().setFromEuler(new THREE.Euler(0,yaw+hash2(i,cz,83)*.55,0));
        const trunkScale=new THREE.Vector3(s*(.92+hash2(i,7)*.15),s*(.96+hash2(i,13)*.11),s*(.92+hash2(i,11)*.15)),crownScale=new THREE.Vector3(s*(.9+hash2(i,17)*.24),s*(.91+hash2(i,23)*.19),s*(.9+hash2(i,29)*.24));
      ...
      }else if(rockIndex<chunk.rockCount){const s=.45+hash2(i*2,cx-cz)*1.05;chunk.rocks.setMatrixAt(rockIndex++,new THREE.Matrix4().compose(new THREE.Vector3(lx,y+.3*s,lz),new THREE.Quaternion().setFromEuler(new THREE.Euler(hash2(i,1)*.8,hash2(i,2)*6.28,hash2(i,3)*.8)),new THREE.Vector3(s*1.35,s*.72,s)));}
    }
