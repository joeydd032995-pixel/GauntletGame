...
const started=Date.now(),marks=[];const mark=name=>marks.push([name,Math.max(.1,(Date.now()-started)/1000)]);let fatal=null,metrics=null;
try{
  await page.goto('http://127.0.0.1:4173',{waitUntil:'networkidle',timeout:30000});await page.waitForSelector('canvas',{state:'visible'});await page.waitForTimeout(3200);mark('01-idle-1080p.png');
  ...
  await page.keyboard.press('Space');await page.waitForTimeout(140);mark('05-evade-1080p.png');await page.waitForTimeout(2200);metrics=await page.evaluate(()=>window.__GAUNTLET_METRICS__||null);
