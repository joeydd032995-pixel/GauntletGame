...
if(!env)failures.push('hybrid environment telemetry missing');
  ...
  for(const required of ['pillar','crates','barrel','brokenWall','archedWall','stairs'])if(!env.assets?.[required])failures.push(`missing authored environment source=${required}`);
  if((env.instances||0)<10)failures.push(`authored environment instances ${env.instances||0}<10`);
