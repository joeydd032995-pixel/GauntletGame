...
let browser,context,page;const errors=[];let captureError=null;
try{
  ...
  for(const[name,opts,delay]of shots){
    ...
    const file=path.join(out,`${name}.png`);await bounded(`${name}: QHD screenshot`,()=>page.screenshot({path:file,type:'png',timeout:SHOT_MS}),SHOT_MS+2_000);const stat=await fs.stat(file);report.push({name,file,width:W,height:H,bytes:stat.size,setup,metrics});
  }
