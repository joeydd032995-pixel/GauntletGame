...
const assertPng=(bytes,label)=>{if(bytes.length<256)throw new Error(`${label} is suspiciously small (${bytes.length} bytes)`);if(bytes.subarray(1,4).toString('ascii')!=='PNG')throw new Error(`${label} is not a valid PNG`);};
...
async function fetchBytes({repo,revision,sourcePath,expectedBytes,label}){
  ...
  const response=await fetch(url,{redirect:'follow'});if(!response.ok)throw new Error(`Pinned authored asset fetch failed ${response.status}: ${url}`);
  const bytes=Buffer.from(await response.arrayBuffer());if(expectedBytes&&bytes.length!==expectedBytes)throw new Error(`Pinned authored asset size mismatch ${label}: ${bytes.length} != ${expectedBytes}`);
  return{url,bytes};
}
...
function pruneGlbAnimations(sourceBytes,keepNames,label){
  ...
  const json=JSON.parse(chunks[jsonIndex].data.toString('utf8').replace(/\u0000+$/g,'').trim()),sourceAnimations=json.animations||[],keep=new Set(keepNames),retained=sourceAnimations.filter(a=>keep.has(a.name)),found=new Set(retained.map(a=>a.name)),missing=keepNames.filter(name=>!found.has(name));if(missing.length)throw new Error(`${label} missing required animations: ${missing.join(', ')}`);json.animations=retained;
...
function stylizeExternalGltf(sourceBytes,label){
  const json=JSON.parse(sourceBytes.toString('utf8')),usedTextureIndices=[];
  for(const material of json.materials||[]){
    delete material.normalTexture;delete material.occlusionTexture;delete material.emissiveTexture;
    const pbr=material.pbrMetallicRoughness||(material.pbrMetallicRoughness={});delete pbr.metallicRoughnessTexture;pbr.metallicFactor=0;pbr.roughnessFactor=Math.max(.74,Number(pbr.roughnessFactor??.78));
    if(Number.isInteger(pbr.baseColorTexture?.index))usedTextureIndices.push(pbr.baseColorTexture.index);
  }
  const unique=[...new Set(usedTextureIndices)],textureMap=new Map(),imageMap=new Map(),newTextures=[],newImages=[];
  for(const oldTextureIndex of unique){const texture=json.textures?.[oldTextureIndex];if(!texture)throw new Error(`${label} missing base-color texture index ${oldTextureIndex}`);const oldImageIndex=texture.source;if(!Number.isInteger(oldImageIndex)||!json.images?.[oldImageIndex])throw new Error(`${label} missing image for texture ${oldTextureIndex}`);if(!imageMap.has(oldImageIndex)){imageMap.set(oldImageIndex,newImages.length);newImages.push(json.images[oldImageIndex]);}textureMap.set(oldTextureIndex,newTextures.length);newTextures.push({...texture,source:imageMap.get(oldImageIndex)});}
  for(const material of json.materials||[]){const ref=material.pbrMetallicRoughness?.baseColorTexture;if(ref)ref.index=textureMap.get(ref.index);}
  json.textures=newTextures;json.images=newImages;
  const requiredImages=newImages.map(image=>image.uri).filter(Boolean);if(!requiredImages.length)throw new Error(`${label} has no retained authored base-color images`);
  return{bytes:Buffer.from(JSON.stringify(json,null,2),'utf8'),requiredImages,materials:(json.materials||[]).map(m=>m.name||'unnamed')};
}
...
async function fetchPinnedGlb({repo,revision,sourcePath,destDir,destName,expectedBytes,role,keepAnimations}){
  const fetched=await fetchBytes({repo,revision,sourcePath,expectedBytes,label:destName});assertGlb(fetched.bytes,destName);const pruned=keepAnimations?pruneGlbAnimations(fetched.bytes,keepAnimations,destName):null,output=pruned?.bytes||fetched.bytes;await mkdir(destDir,{recursive:true});await writeFile(path.join(destDir,destName),output);
...
async function fetchPinnedOutfit({repo,revision,basePath,name,expectedGltfBytes,expectedBinBytes,imageSizes,destDir,provenance,writtenImages}){
  const gltfSource=`${basePath}/${name}.gltf`,binSource=`${basePath}/${name}.bin`,gltf=await fetchBytes({repo,revision,sourcePath:gltfSource,expectedBytes:expectedGltfBytes,label:`${name}.gltf`}),bin=await fetchBytes({repo,revision,sourcePath:binSource,expectedBytes:expectedBinBytes,label:`${name}.bin`}),stylized=stylizeExternalGltf(gltf.bytes,`${name}.gltf`);
  ...
  for(const image of stylized.requiredImages){if(writtenImages.has(image))continue;const expectedBytes=imageSizes[image];if(!expectedBytes)throw new Error(`No pinned byte size for required outfit image ${image}`);const sourcePath=`${basePath}/${image}`,asset=await fetchBytes({repo,revision,sourcePath,expectedBytes,label:image});assertPng(asset.bytes,image);await writeFile(path.join(destDir,image),asset.bytes);writtenImages.add(image);provenance.files[image]={role:'retained authored base-color texture',url:asset.url,repository:repo,revision,sourcePath,bytes:asset.bytes.length,sha256:sha256(asset.bytes)};}
}
...
async function installPinnedQuaterniusDressed(){
  ...
  const ualCore=['Idle_Loop','Walk_Loop','Jog_Fwd_Loop','Sprint_Loop','Punch_Cross','Spell_Simple_Shoot','Hit_Chest','Death01','Roll','Sword_Attack'],ualCombat=['Idle_Shield_Loop','Melee_Hook','Hit_Knockback','Slide_Start'];
  const imageSizes={T_Ranger_BaseColor.png:6_630_927,T_Regular_Male_Dark_BaseColor.png:1_316_151,T_Regular_Female_Dark_BaseColor.png:1_348_542};
  const outDir=path.join(OUT_ROOT,'quaternius-dressed');await mkdir(outDir,{recursive:true});const provenance={source:'Quaternius Modular Character Outfits - Fantasy + Universal Animation Library + Knight equipment, pinned redistributed CC0 copies',officialSources:['https://quaternius.com/packs/modularcharacteroutfitsfantasy.html','https://quaternius.com/packs/universalbasecharacters.html','https://quaternius.com/packs/universalanimationlibrary.html'],license:'CC0 1.0',purpose:'Dressed OSRS-directed character tier. Universal anatomical topology/rig lineage retained; nude base presentation is forbidden.',files:{}};const writtenImages=new Set();
  await fetchPinnedOutfit({repo:OUTFIT_REPO,revision:OUTFIT_REV,basePath:OUTFIT_BASE,name:'Male_Ranger',expectedGltfBytes:41_447,expectedBinBytes:1_746_884,imageSizes,destDir:outDir,provenance,writtenImages});
  await fetchPinnedOutfit({repo:OUTFIT_REPO,revision:OUTFIT_REV,basePath:OUTFIT_BASE,name:'Female_Ranger',expectedGltfBytes:45_461,expectedBinBytes:1_959_556,imageSizes,destDir:outDir,provenance,writtenImages});
  const glbs=[
  ...
  const profile={schemaVersion:2,target:'high-end OSRS/07Scape',tier:'quaternius-dressed-intermediate',provider:'Quaternius CC0 (pinned redistributed copies)',visualAcceptanceEligible:false,clothingComplete:true,visualBlocker:'Near-nude presentation eliminated with rigged fantasy clothing and equipment. Harsh OSRS visual review must still approve silhouette, fit, animation deformation, and role readability before production eligibility.',actors:{
    hero:{url:'/assets/authored/quaternius-dressed/Male_Ranger.gltf',label:'Vanguard / Quaternius Ranger dressed body',height:2.48,bulk:1.04,animationUrls,clothingComplete:true,silhouetteClass:'hooded-ranger-vanguard',hiddenMeshes:[],palette:'weathered-forest-bronze',equipment:[{name:'Vanguard sword',url:'/assets/authored/quaternius-dressed/VanguardSword.glb',bone:'hand_r',rootPosition:[.85,1.35,-.2],rootRotation:[-.5,-2,-.785398],rootScale:1}]},
    enemy:{url:'/assets/authored/quaternius-dressed/Female_Ranger.gltf',label:'Dread Warden / Quaternius Ranger armored variant',height:2.72,bulk:1.13,animationUrls,clothingComplete:true,silhouetteClass:'helmeted-heavy-warden',hiddenMeshes:['Female_Ranger_Head_Hood'],palette:'charcoal-oxblood-iron',equipment:[{name:'Warden helmet',url:'/assets/authored/quaternius-dressed/WardenHelmet.glb',bone:'Head',rootPosition:[0,5,-.1],rootRotation:[0,0,0],rootScale:1.08},{name:'Warden club',url:'/assets/authored/quaternius-dressed/WardenClub.glb',bone:'hand_r',rootPosition:[.85,1.35,-.2],rootRotation:[-.5,-2,-.785398],rootScale:1.12}]}
  }};
  await writeFile(PROFILE_PATH,JSON.stringify(profile,null,2));console.log(`Pinned Quaternius dressed tier ready: two rigged fantasy outfits, three authored equipment pieces and ${ualCore.length+ualCombat.length} Gauntlet-relevant authored animations. Nude/base-body presentation is no longer an allowed default.`);
}

await mkdir(OUT_ROOT,{recursive:true});if(QUATERNIUS_MANIFEST)await installQuaternius(QUATERNIUS_MANIFEST);else await installPinnedQuaterniusDressed();
