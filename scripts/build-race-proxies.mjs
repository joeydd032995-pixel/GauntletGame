...
const GENERATOR_VERSION='2.0.1-production-rigid';
...
const LODS={hero:{radial:18,sphere:2,segments:3,detail:1,scale:1},mid:{radial:11,sphere:1,segments:2,detail:.72,scale:1},far:{radial:7,sphere:1,segments:1,detail:.42,scale:1}};
...
function makeRig(def,lod,M){
  ...
  for(const[side,sgn]of[['L',-1],['R',1]]){
    ...
    const fore=group(upper,`forearm_${side}`,[0,-upperLen,0]);add(fore,cyl(`forearm_${side}_mesh`,.074*def.bulk,.058*def.bulk,foreLen,M.skin,q.radial,q.segments),[0,-foreLen/2,0]);
    const hand=group(fore,`hand_${side}`,[0,-foreLen,0]);add(hand,ico(`hand_${side}_mesh`,[.068*def.bulk,.09,.058],M.skin,q.sphere),[0,-.04*scale,0]);
  }
  ...
  for(const[side,sgn]of[['L',-1],['R',1]]){
    ...
    const shin=group(thigh,`shin_${side}`,[0,-thighLen,0]);add(shin,cyl(`shin_${side}_mesh`,.088*def.bulk,.067*def.bulk,shinLen,M.leather,q.radial,q.segments),[0,-shinLen/2,0]);
    const foot=group(shin,`foot_${side}`,[0,-shinLen,0]);add(foot,wedge(`foot_${side}_mesh`,.17*def.bulk,.115*scale,.30*scale,M.leather),[0,-.04*scale,.07*scale]);
  }
  return{root,pelvis,abdomen,torso,chestBand,belt,head,crest,scale,q,M};
}

function decorateCairnborn(c,lod){const{torso,head,crest,M,q}=c;add(torso,plate('ancestral_chest_plate',.76,.43,.10,M.primary,10),[0,.02,-.32]);add(torso,plate('lower_slate_plate',.58,.28,.085,M.secondary,8),[0,-.17,-.29]);add(head,plate('carved_faceplate',.31,.28,.085,M.primary,8),[0,.15,-.205]);add(head,box('brow_inlay',[.29,.052,.035],M.metal,1),[0,.21,-.225]);for(const x of[-.105,0,.105])add(torso,box(`rune_channel_${x}`,[.024,.30,.028],M.accent,1),[x,.015,-.37]);for(const[side,sgn]of[['L',-1],['R',1]])add(torso,ico(`layered_pauldron_${side}`,[.20,.14,.17],M.primary,q.sphere),[sgn*.47,.17,0]);if(lod!=='far'){for(let i=0;i<4;i++)add(torso,plate(`slate_scale_${i}`,.19,.13,.035,i%2?M.secondary:M.primary,7),[(i-1.5)*.16,-.07+(i%2)*.08,-.355]);add(crest,cone('replacement_stone_crystal',.065,.20,M.tertiary,8),[-.12,.06,.02],[0,0,deg(-15)]);}if(lod==='hero')for(let i=0;i<8;i++){const a=i/8*Math.PI*2;add(torso,box(`carved_edge_${i}`,[.035,.12,.035],i%2?M.metal:M.secondary,1),[Math.sin(a)*.31,.02+Math.cos(a)*.17,-.355]);}}
function decorateBrinesworn(c,lod){const{torso,head,crest,M,q}=c;for(const[side,sgn]of[['L',-1],['R',1]]){add(head,fin(`fin_ear_${side}`,sgn,M.tertiary,lod==='hero'?.032:.024),[sgn*.145,.15,0]);add(torso,ico(`shell_pauldron_${side}`,[.18,.11,.15],M.secondary,q.sphere),[sgn*.42,.17,0]);if(lod!=='far')for(let i=0;i<3;i++)add(torso,plate(`mineral_ridge_${side}_${i}`,.13,.065,.035,M.primary,7),[sgn*(.23+i*.045),.12-i*.12,-.30],[0,0,deg(sgn*(12+i*4))]);}add(torso,box('harness_L',[.07,.62,.052],M.leather,1),[-.14,0,-.35],[0,0,deg(-17)]);add(torso,box('harness_R',[.07,.62,.052],M.leather,1),[.14,0,-.35],[0,0,deg(17)]);add(head,ico('sea_glass_eye_L',[.035,.045,.025],M.accent,q.sphere),[-.055,.17,-.175]);add(head,ico('sea_glass_eye_R',[.035,.045,.025],M.accent,q.sphere),[.055,.17,-.175]);add(crest,plate('navigation_circlet',.29,.08,.03,M.metal,8),[0,-.09,-.10]);if(lod==='hero')for(let i=0;i<6;i++)add(torso,ico(`salt_nodule_${i}`,[.035,.025,.026],i%2?M.secondary:M.accent,1),[-.22+i*.09,-.18+(i%2)*.08,-.34]);}
function decorateMyceliad(c,lod){const{torso,head,crest,M,q}=c;add(crest,ico('mushroom_crown',[.40,.12,.34],M.primary,q.sphere),[0,.08,0]);add(crest,ico('mushroom_gills',[.33,.065,.28],M.secondary,q.sphere),[0,.005,0]);add(torso,ico('grown_chest_plate',[.36,.27,.25],M.leather,q.sphere),[0,.01,-.04]);for(const[side,sgn]of[['L',-1],['R',1]])add(torso,ico(`shelf_fungus_${side}`,[.20,.06,.15],M.secondary,q.sphere),[sgn*.39,.18,.03],[0,0,deg(sgn*8)]);if(lod!=='far'){for(let i=0;i<5;i++){const s=i%2?-1:1;add(torso,cone(`root_spur_${i}`,.045,.20,M.leather,7),[s*(.20+i*.025),-.18+i*.08,.05],[0,0,deg(s*35)]);}for(let i=0;i<4;i++)add(torso,ico(`spore_node_${i}`,[.03,.03,.03],M.accent,1),[-.16+i*.11,.05+(i%2)*.09,-.31]);}if(lod==='hero')for(let i=0;i<8;i++){const a=i/8*Math.PI*2;add(crest,plate(`gill_lamella_${i}`,.11,.035,.018,M.secondary,6),[Math.cos(a)*.22,.01,Math.sin(a)*.16],[0,a,0]);}}
function decorateVeylkin(c,lod){const{torso,head,crest,M,q}=c;for(const[side,sgn]of[['L',-1],['R',1]]){add(head,ico(`luminous_eye_${side}`,[.036,.045,.025],M.accent,q.sphere),[sgn*.055,.17,-.175]);const stalk=group(crest,`sensory_crest_${side}`,[sgn*.06,-.02,0]);stalk.rotation.z=deg(-sgn*25);add(stalk,cyl(`sensory_crest_${side}_mesh`,.016,.022,.34,M.tertiary,Math.max(6,q.radial/2),1),[0,.17,0]);add(stalk,ico(`crest_tip_${side}`,[.04,.055,.04],M.accent,q.sphere),[0,.35,0]);add(torso,membrane(`moth_mantle_${side}`,sgn,M.primary,lod==='hero'?1.12:.92),[sgn*.05,.17,.01]);add(torso,membrane(`vestigial_membrane_${side}`,sgn,M.secondary,lod==='hero'?.82:.62),[sgn*.03,.03,.16],[0,0,deg(sgn*8)]);add(torso,ico(`mantle_anchor_${side}`,[.14,.09,.13],M.primary,q.sphere),[sgn*.39,.19,0]);}add(torso,plate('moon_chest_clasp',.20,.12,.045,M.metal,8),[0,.16,-.34]);if(lod==='hero')for(let i=0;i<6;i++)add(torso,plate(`powder_mark_${i}`,.06,.035,.012,M.tertiary,6),[-.15+i*.06,-.12+(i%2)*.09,-.355]);}
function decorateEchoed(c,lod){const{torso,head,crest,M,q}=c;add(torso,plate('bastion_shoulder_fragment',.28,.24,.09,M.primary,8),[-.32,.16,-.06],[0,0,deg(-8)]);add(torso,ico('verdant_shoulder_growth',[.19,.11,.15],M.secondary,q.sphere),[.34,.17,.02]);add(head,ico('asymmetrical_iris',[.038,.032,.021],M.accent,q.sphere),[.05,.17,-.18]);for(let i=0;i<3;i++)add(torso,cone(`impossible_relic_${i}`,.045,.18+i*.025,M.metal,7),[-.17+i*.16,.03+i*.08,-.34],[deg(12*i),0,deg(-18+18*i)]);add(crest,plate('lost_civilization_diadem',.27,.09,.035,M.tertiary,8),[0,-.05,-.10]);if(lod!=='far')for(let i=0;i<4;i++)add(torso,box(`timeline_displacement_${i}`,[.018,.20-.02*i,.028],M.accent,1),[-.15+i*.10,-.10+i*.08,.25]);if(lod==='hero'){const echoMat=M.accent.clone();echoMat.name='second_possibility_echo';echoMat.transparent=true;echoMat.opacity=.22;echoMat.depthWrite=false;add(torso,ico('duplicate_silhouette_shard',[.27,.24,.08],echoMat,2),[.045,.02,.18],[0,deg(8),deg(4)]);for(let i=0;i<6;i++)add(torso,plate(`fracture_token_${i}`,.07,.05,.018,i%2?M.metal:M.accent,6),[-.18+i*.07,.20-(i%3)*.12,-.36]);}}
...
for(const def of defs){const record={key:def.key,label:def.label,faction:def.faction,elementId:def.elementId,height:def.height,productionMesh:false,rigType:'articulated-rigid-part',generatorVersion:GENERATOR_VERSION,silhouetteNotes:def.silhouetteNotes,parts:[...REQUIRED_PARTS],triangles:{},meshes:{},materials:[]};for(const lod of['hero','mid','far']){const M=createMaterials(def.key);const c=makeRig(def,lod,M);def.decorate(c,lod);const model=c.root;model.userData={...model.userData,race:def.label,faction:def.faction,elementId:def.elementId,target:ART,rigType:'articulated-rigid-part',productionMesh:false,lod};validateParts(model,def,lod);const stats=measure(model);if(lod==='hero'&&stats.triangles<3000)throw new Error(`${def.key} hero below 3000 triangles: ${stats.triangles}`);const suffix=lod==='hero'?'':`_${lod}`;const filename=`gauntlet_${def.key}${suffix}_v1.glb`;await exportGlb(model,filename);record.triangles[lod]=stats.triangles;record.meshes[lod]=stats.meshes;if(lod==='hero')record.materials=stats.materials;record[`${lod}Url`]=`/assets/races/${filename}`;}record.url=record.heroUrl;manifest.races.push(record);}
